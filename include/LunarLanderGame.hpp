...
//Line 104: idea from chatGPT (defining the parameters in the constructor)
...
public:
    ...
    void update(const float dt) {
        ...
        if (crash == true) {
            if (time < timer1 + 2) {
                ...
                if (!moonScene.isInScene(HUD.getCenterText())) {
                    HUD.setCenterText("Destroyed - You crashed", 1);
                }
        ...
        if (landing == true) {
            if (time < timer1 + 2) {
                if (!moonScene.isInScene(HUD.getCenterText())) {
                    std::string scoreText = std::to_string(static_cast<int>(points));
                    HUD.setCenterText("Congratulations - You got " + scoreText + " points", 1);
                }
                moonScene.addToScene(HUD.getCenterText());
                return;
            } else {
        ...
        if (!collision) {
            ...
            if (static_cast<int>(time*100)%20 < 1) {
                //setting the altitude to the bottom of the ship - the closest lines height.
        ...
        if (lunarLander.getPosition()[0] > surfaceBorder[1]-(surfaceLength+20)) {
            ...
            //Adding a new 'landscape' instance at the left border
